<form action="{{url('/xin/update/'.$res->xin_id)}}"method="post"enctype="multipart/form-data">
	@csrf
	<table>
		<meta name="csrf-token" content="{{ csrf_token() }}">
		<input type="hidden" value="{{$res->xin_id}}">
		<tr>
			<td>姓名</td>
			<td><input type="text" name="xin_name"id="xin_name"value="{{$res->xin_name}}">
				<span>{{$errors->first('xin_name')}}</span>
			</td>
		</tr>
		<tr>
			<td>年龄</td>
			<td><input type="text" name="xin_age"id="xin_age"value="{{$res->xin_age}}">
				<span>{{$errors->first('xin_age')}}</span>
			</td>
		</tr>
		<tr>
			<td>身份证</td>
			<td><input type="text" name="xin_care"id="xin_care"value="{{$res->xin_care}}">
